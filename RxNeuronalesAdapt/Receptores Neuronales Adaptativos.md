...
Abstract — Sixth-generation (6G) wireless systems impose unprecedented requirements: sub-millisecond latency for ultra-reliable low-latency communications (URLLC), reliability of 99.99999%, and device densities reaching $10^7$ devices/km$^2$. Traditional physical-layer algorithms (MMSE, ZF) cannot simultaneously meet these constraints while approaching information-theoretic limits. This article proposes a hybrid CNN-Transformer adaptive neural receiver that integrates three innovations: (i) a Joint Source-Channel Coding (JSCC) variational autoencoder achieving 8$\times$ compression (128-D to 16-D latent space), corresponding to 4$\times$ effective bandwidth reduction relative to a 64-D baseline; (ii) multi-head temporal attention for channel estimation, eliminating interpolation error floors in high-Doppler scenarios; and (iii) a hierarchical three-module early-exit architecture co-optimized via a multi-objective cost function $\mathcal{L}(\boldsymbol{\theta}, \mathbf{q}) = \lambda_1 \cdot \text{BER}(\boldsymbol{\theta}) + \lambda_2 \cdot \text{Latency}(\boldsymbol{\theta}, \mathbf{q}) + \lambda_3 \cdot \text{Complexity}(\mathbf{q})$. Multi-stage compression (QAT INT8/INT4, 70% structured pruning, progressive knowledge distillation) yields 94% FLOPs and 87% memory reduction. Analytical estimates based on the Roofline performance model project 0.73 ms inference latency on NVIDIA Jetson AGX Orin and 0.58 ms on FPGA Zynq UltraScale+. BER simulations confirm a 2.1 dB gain over MMSE at $10^{-3}$ for 4$\times$4 MIMO, 16-QAM over CDL-C channels. Deep reinforcement learning (DRL) orchestration via PPO+MAML is proposed as a theoretical framework; experimental validation remains future work.
...
5. **DRL Orchestration Framework (Theoretical)**: A PPO+MAML reinforcement learning agent for joint selection of receiver module, compression level, and offloading target based on real-time SNR, battery state, and latency budget. Experimental validation of this component is left as future work.
...
The JSCC module consists of a variational autoencoder (VAE) encoder $E_\phi: \mathbb{R}^{128} \to \mathbb{R}^{16}$ and decoder $D_\psi: \mathbb{R}^{16} \to \mathbb{R}^{128}$. The encoder maps the 128-dimensional received feature vector (extracted from the first ResNet block) to a 16-dimensional latent code $\mathbf{z}$, achieving 8$\times$ compression relative to the input dimension. Compared to an alternative 64-D intermediate representation used in baseline JSCC systems, the proposed 16-D latent space yields an additional 4$\times$ effective bandwidth reduction, for a net 4$\times$ overhead reduction in channel transmission costs (or equivalently, 8$\times$ compression vs. the raw 128-D representation). This distinction is important: the 8$\times$ figure refers to the absolute compression ratio, while the 4$\times$ figure refers to the effective communication overhead reduction relative to the 64-D baseline commonly used in prior JSCC work [5].
