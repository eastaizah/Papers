...
16. **Figure 3 caption (continued)**: A note has been added clarifying the figure numbering: *"Note: the script internally labels the output as 'Experiment 3 – Complete BER vs Eb/N0 curves' with a legacy reference to 'Figure 5'; the correct figure number in this article is **Figure 3**."* The file `run_ber_curves.py` is not present in the current repository snapshot reviewed here; the label correction should be applied in the external simulation repository when the script is next updated.
