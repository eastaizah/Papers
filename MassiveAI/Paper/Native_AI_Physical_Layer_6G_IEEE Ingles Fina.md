...
The pioneering work of O'Shea and Hoydis introduced the concept of the **autoencoder for end-to-end communications**, where both the transmitter and receiver are implemented as jointly trained neural networks to minimize a loss function related to the error rate [10]. This approach demonstrated that neural systems could learn modulation and coding schemes competitive with traditional designs, and in some cases discover unconventional solutions with superior performance — provided that sufficient training resources, diverse datasets, and channel-matched training conditions are available.

However, a critical and evidence-based assessment of the field is essential. Several fundamental limitations temper enthusiasm and remain active areas of research:

- **Generalisation across channel distributions**: Models trained on one channel type (e.g., AWGN) often fail to maintain performance under distributional shift (e.g., Rayleigh fading, non-stationary channels). Section III.A.8 provides concrete BER evidence of this degradation.
- **Sample inefficiency**: Neural PHY models typically require orders of magnitude more training samples than classical algorithms to achieve comparable performance. The proof-of-concept benchmark in Section III.A.7 illustrates that even 100,000 training epochs on CPU are insufficient for full convergence in short block-length autoencoder coding.
- **Interpretability**: Deep networks for PHY are largely black boxes. Unlike traditional algorithms with proven convergence guarantees and analyzable error floors, the failure modes of neural PHY are difficult to predict or certify — a critical barrier for deployment in safety-critical applications (Sections VI.B).
- **Hardware constraints**: The inference latency and energy consumption of neural PHY models currently exceed those of hardware-optimised classical algorithms by 1–2 orders of magnitude, a gap that closes only with aggressive quantization, pruning, and custom silicon (Sections V.A.3, V.D, and VI; see also energy-performance trade-offs in Table IV).

These limitations do not diminish the transformative potential of native AI for 6G, but they do underscore that the path from theoretical promise to production deployment requires sustained progress on generalization, efficiency, and certification — dimensions this survey addresses throughout.
...
> **Note on Scope (Layers 2–3):** This survey focuses on the physical layer (Layer 1). Native AI also plays an increasingly important role in Layer 2–3 procedures in 6G, including handover decision-making (predictive mobility management), MAC-layer scheduling, interference coordination, and routing. These procedures are being addressed in 3GPP Release 18/19 under the NWDAF and AI-RAN frameworks. Although a full treatment with the mathematical rigour applied to PHY in this survey is beyond our scope, the reader is referred to [39]–[40] for AI-based scheduling and multi-cell coordination, and to Section VI.I for the 3GPP architecture that governs AI across all layers.

> **Figure 1.** Architectural Comparison: Traditional Communication System vs. AI-Native System (End-to-End Autoencoder).
>
> *[FIGURE_PENDING — Description for subsequent generation: Comparative block diagram in two parallel rows. **Top row (Traditional System):** horizontal cascade diagram with labeled blocks: Source → Source Encoder (removes redundancy, compression) → Channel Encoder (adds controlled redundancy for error correction, rate R=k/n) → Modulator (maps bits to symbols in signal space, e.g., QAM) → Channel (cloud labeled "H + n", representing fading matrix H and additive noise n~CN(0,σ²I)) → Demodulator (maps received symbols back to soft bits) → Channel Decoder (corrects errors via belief propagation or Viterbi) → Source Decoder (reconstructs original source) → Destination. Each block is independently optimized. Annotation: "Sequential block-wise design: each block locally optimal ≠ globally optimal". **Bottom row (AI-Native System):** simplified diagram with three main blocks: Source (s) → Transmitter Neural Network fθ (single DNN jointly performing source encoding, channel encoding and modulation; input: message s, output: complex signal x∈ℂ^M satisfying power constraint E[‖x‖²]≤P) → Channel (cloud with H and n, shared with traditional row) → Receiver Neural Network gφ (single DNN jointly performing demodulation, channel decoding and source decoding; input: received signal y, output: estimate ŝ) → Destination (ŝ). Bidirectional arrows labeled ∇θ and ∇φ indicate end-to-end gradient flow during joint training. Annotation: "Joint end-to-end optimisation: min_{θ,φ} E[L(s, gφ(h(fθ(s))+n))]". **Color coding:** traditional system blocks in light gray; AI-Native transmitter (fθ) in dark blue; AI-Native receiver (gφ) in dark blue; channel block in both rows in orange. **Key label at bottom:** "Traditional: 7 separate blocks, each independently designed | AI-Native: 2 neural networks, jointly trained end-to-end". Suggested size: two IEEE columns, 3.5 inches tall.]*
...
>
> *[FIGURE_PENDING — Description for later generation: Hierarchical mind map tree with central root "Native AI in PHY 6G". **Five distinct main branches** (each appears exactly once): (1) Neural Channel Coding [sub-branches: Autoencoder, RNN Decoder, Algorithm Unrolling, Adaptive Codes]; (2) Channel Estimation [sub-branches: CNN/U-Net, Bidirectional LSTM, Transformer/ChannelFormer, Diffusion Models, Physics-Informed NN]; (3) Signal Detection (Multi-User) [sub-branches: DetNet, LISTA, RL-based Detector, Model-Driven]; (4) Intelligent Beamforming [sub-branches: Black-box NN, WMMSE Unrolling, Multi-Agent RL, Predictive BF]; (5) Radio Resource Management [sub-branches: DQN Scheduling, GNN Multi-Cell, Game-Theoretic, Cross-Layer RL]. **Note:** Branch (3) "Signal Detection (Multi-User)" appears **only once** in this figure and is distinct from Branch (4) "Intelligent Beamforming" — do not duplicate. Additionally, a transversal branch "Foundation Models (2024–2025)" [sub-branches: Universal Channel FM, LLM for RRM, Specialized Transformers] connects to all main branches via dashed arrows. Color coding by technology family: blue for classical components, green for end-to-end/cross-layer systems, orange for Foundation Models. Suggested size: full IEEE page, portrait format.]*
...
>
> *[FIGURE_PENDING — Description for later generation: IEEE-style neural network architecture diagram. **Important layout notes to avoid identified readability issues:** (a) All axis labels and layer labels must use **horizontal text only** — no vertical/rotated text, as vertical text at the left side of this figure is unreadable in print; (b) The upper-left corner of the figure must include a clear legend box identifying the block naming convention (Transmitter=red, Channel=gray, Receiver=blue); (c) The arrow between "Transmitter" and "Channel" must be labeled explicitly: top of arrow → "$\mathbf{x} \in \mathbb{C}^{N_t \times n}$ (transmitted signal, power-normalized)"; below the arrow → "Power constraint: $\mathbb{E}[\|\mathbf{x}\|^2] \leq P$". Left block **"TRANSMITTER (fθ)"**: input message s (M-dimensional one-hot) → Embedding Layer [M→128, label: "Message Embedding"] → Dense Layer [128→256, ReLU, label: "FC-1"] → Dense Layer [256→512, ReLU, label: "FC-2"] → Dense Layer [512→Nt×n×2, label: "FC-3"] → Reshape [Nt×n, complex, label: "I/Q Reshape"] → Power Normalization [÷√(E[|x|²]), label: "Power Norm."] → output x ∈ ℂ^{Nt×n} with label "x (channel input)". Central block **"CHANNEL (Rayleigh MIMO)"**: Matrix multiplication H [Nr×Nt, Rayleigh, label: "H∼CN(0,I)"] → AWGN Noise Addition n [CN(0,σ²I), label: "n∼CN(0,σ²I)"] → output y ∈ ℂ^{Nr×n} with label "y (channel output)". Right block **"RECEIVER (gφ)"**: input y → I/Q Separation and Flatten [2Nr·n, label: "I/Q Split + Flatten"] → Dense Layer [512, ReLU, label: "FC-4"] → Dense Layer [256, ReLU, label: "FC-5"] → Dense Layer [128, ReLU, label: "FC-6"] → Dense Layer [M, Softmax, label: "Output Softmax"] → output ŝ with label "ŝ (estimated message)". Loss and gradient arrows: "L = CrossEntropy(s, ŝ)" with **horizontal** gradient arrows labeled "∇θ (backprop through fθ)" and "∇φ (backprop through gφ)" and a note "Differentiable channel: reparameterization trick used for H, n". Colors: red for transmitter blocks, blue for receiver blocks, gray for channel block. Suggested size: two IEEE columns, 4 inches tall.]*
...
>
> *[FIGURE_PENDING — Description for later generation: Composite figure with two subfigures. **Note: the PSNR curves in subfigure (b) are reproduced from the literature results of [45],[48] and are not AI-generated; they represent published simulation data from the referenced papers, not output from an AI image-generation tool.** Subfigure (a) — Architecture (top): System architecture diagram of JSCC with input image (224×224×3 RGB, example CelebA image) → CNN Encoder (3 convolutional layers with stride 2, reduction H/8×W/8×256) → Flatten + Dense → n complex symbols → Wireless channel (cloud with H, n, SNR label) → Dense + Reshape → CNN Decoder (3 TransposedConv layers, upsampling to H×W×3) → Reconstructed image. Show the classical cascade in parallel: Image → JPEG/BPG (compressor) → Bits → LDPC/Polar (channel encoder) → Channel → Channel Decoder → JPEG/BPG (decompressor) → Image. Indicate with brackets "Joint Optimization" for the JSCC system. Subfigure (b) — PSNR Curves (bottom): Plot with PSNR (dB) on Y-axis (15–35 dB) vs. SNR (dB) on X-axis (−2 to 18 dB). Three curves: JPEG+LDPC [blue, circles], BPG+Polar [green, triangles], JSCC-Neural [orange, diamonds]. Fixed channel rate: 0.1 symbols/pixel. Dataset: CelebA, 64×64 images. Title: "PSNR vs. Channel SNR, Rate=0.1 s/px, CelebA-64×64". The JSCC curve shows superiority especially at low SNR (cliff effect avoided). Suggested size: two IEEE columns, 4.5 inches tall.]*
...
**Distinction from Representation Learning**: It is important to clarify the relationship between semantic communications and representation learning, as these concepts are frequently conflated. *Representation learning* (Section II.C) is a general machine learning technique by which neural networks learn compact, task-relevant feature representations from raw data — it is agnostic to the communication channel and does not, per se, target transmission efficiency. *Semantic communications*, by contrast, is an end-to-end system design philosophy that exploits representation learning *within the communication pipeline* to transmit only task-relevant information across a noisy channel. The key distinctions are: (a) semantic communications jointly optimise the representation and the channel transmission, whereas standalone representation learning optimises only the feature space; (b) semantic communications explicitly model the channel and power constraints; (c) the goal of semantic communications is to improve transmission efficiency (bits per task-relevant unit), while representation learning targets reconstruction or classification fidelity. In the formalism below, the semantic extraction function $f_{\text{sem}}$ performs representation learning, but the full semantic communication system adds the channel encoder $f_{\text{enc}}$, the physical channel, and the task-oriented loss — elements absent from representation learning frameworks.
...
#### 3) Quantization, Model Optimization, and Hardware-Aware Learning
...
**Hardware-Aware Learning: RF Impairments and Practical Transceiver Constraints**

A critical but often underexplored dimension of neural PHY design is *hardware-aware learning* — the systematic incorporation of RF hardware impairments into the training process. Practical transceivers exhibit impairments that are absent from idealized channel models, including:

- **Power Amplifier (PA) Non-linearity**: PAs operate near saturation for energy efficiency, introducing amplitude-dependent distortion characterised by the AM/AM and AM/PM curves. Neural transmitter networks that do not account for PA non-linearity produce constellations with high PAPR (Peak-to-Average Power Ratio) that suffer severe degradation in practice. Hardware-aware training incorporates a differentiable PA model $g_{\text{PA}}(x) = A(|x|)e^{j[\angle x + \Phi(|x|)]}$ as an additional layer between the transmitter network and the channel, enabling the network to learn low-PAPR signal representations [9].
- **Phase Noise**: Local oscillator phase noise (characterised by the spectral density $S_{\phi}(f)$) introduces time-varying phase rotations, particularly severe at mmWave and sub-THz frequencies targeted by 6G. Transformer-based receivers with time-frequency attention have shown robustness to phase noise by implicitly learning to track and compensate the rotation [96].
- **I/Q Imbalance**: Mismatches between in-phase and quadrature branches of the RF front-end introduce gain and phase imbalance. End-to-end autoencoder training on data collected through a real RF front-end implicitly adapts to these impairments without requiring explicit calibration [9].
- **Quantization at the ADC**: Finite-resolution analog-to-digital conversion (typically 3–6 bits in energy-constrained 6G receivers) introduces significant non-linear distortion. The straight-through estimator [6] enables backpropagation through the non-differentiable quantization step, allowing neural receivers to jointly learn signal representations and quantizer-aware decoding.
- **DAC Constraints**: Similarly, finite DAC resolution at the transmitter must be modelled during training. 1-bit massive MIMO transmitters [26] are an extreme case where hardware-aware neural beamforming has demonstrated competitive sum-rate relative to full-resolution counterparts.

**Hardware impairments as a known limitation**: The sensitivity of AI-native PHY to RF hardware impairments — particularly when models are trained on ideal simulations and deployed on real hardware (the *sim-to-real gap*) — is a primary obstacle to practical deployment. This sensitivity is more pronounced for end-to-end autoencoders than for classical algorithms whose analytical structures are inherently impairment-agnostic. Addressing this through hardware-in-the-loop training [9], [43] or learned impairment calibration is a prerequisite for 6G deployment readiness.
...
#### 4) Energy Efficiency as a 6G Design Requirement

Energy efficiency is not merely a hardware optimization concern for AI-native PHY — it is a first-class 6G design requirement. ITU-R IMT-2030 [81] specifies that 6G networks must achieve a 100× improvement in network energy efficiency relative to 5G NR. This requirement has three direct implications for native AI at the PHY:

**Training-time energy**: Large-scale PHY model training (e.g., a Foundation Model for universal channel estimation) can consume the equivalent of hundreds of kWh per training run. Federated learning (Section V.B.3) and transfer learning from pre-trained models are essential to amortise this cost across the operator ecosystem.

**Inference-time energy in IoT and edge devices**: 6G use cases include $10^6$–$10^7$ IoT devices/km² [81], most with energy budgets of milliwatts. Neural PHY inference must target < 1 nJ/bit on such devices. Current architectures fall short by 2–3 orders of magnitude without aggressive quantization (INT4/INT2) and hardware-software co-design. Neural Architecture Search (NAS) with energy as a joint objective with BER performance is an active research direction [77].

**System-level energy trade-off**: AI-native PHY may achieve the same BER at lower SNR (requiring lower transmit power), but the energy saved in the RF power amplifier must exceed the energy consumed by the neural inference engine. This system-level energy balance has not been rigorously characterised in the literature and constitutes an important open problem for 6G deployment.

**Quantitative gap**: Table IV summarises computational complexity comparisons; the energy column (not shown) would reflect an additional 10–100× overhead for neural methods on current hardware. Hardware-software co-design targeting ≤ 10 pJ/bit for neural PHY inference is a prerequisite for widespread 6G deployment.
...
### Preamble: Critical Assessment of Known Limitations

Before detailing specific open challenges, a critically balanced assessment of the known limitations of AI-native PHY methods is warranted. The following limitations are well-documented in the literature and represent genuine obstacles to deployment — not merely incremental research gaps:

**1. Generalisation across channel distributions.** AI-native PHY models trained on a specific channel distribution (e.g., AWGN, i.i.d. Rayleigh) exhibit significant performance degradation under distributional shift — i.e., when deployed on channels outside the training distribution. Section III.A.8 (Table III) provides quantitative evidence: an AWGN-trained autoencoder achieves BER ≈ 0.35 at 20 dB SNR under Rayleigh fading, an error floor that never approaches $10^{-3}$. This is in contrast to classical Polar or LDPC codes whose performance degrades gracefully with channel mismatch. Overcoming this limitation requires either channel-agnostic training across a broad set of distributions (at the cost of much larger datasets) or reliable online adaptation (Section VI.F) — neither of which is currently mature for 6G deployment timelines.

**2. Sample inefficiency.** Deep neural networks for PHY typically require orders of magnitude more training data than classical algorithms need for design. An LDPC code can be designed analytically for a target BER in milliseconds; an equivalent autoencoder requires millions of gradient steps. Even when training data is available in simulation, the cost of data collection in hardware-in-the-loop scenarios is prohibitive. This inefficiency is exacerbated in short block-length regimes relevant to URLLC, as demonstrated in Table II (Section III.A.7), where 100,000 epochs of CPU training fail to converge for $n \geq 16$.

**3. Interpretability and certification.** Traditional physical layer algorithms (Viterbi, LDPC belief propagation, MMSE) have analytic performance guarantees: their error floors, waterfall regions, and failure modes can be formally characterised. Neural PHY models are, for the most part, black boxes whose failure modes are unpredictable and uncertifiable. This is a blocking obstacle for deployment in safety-critical 6G applications (URLLC for industrial automation, autonomous vehicles, emergency services) where regulators require performance guarantees that neural networks cannot currently provide. Section VI.B discusses interpretability techniques, but none achieves the formal certification of classical algorithms.

**4. Hardware and energy constraints.** As quantified in Section V.D, the energy consumption of neural inference is 1–2 orders of magnitude higher than hardware-optimised classical algorithms. For battery-powered 6G IoT devices, this energy gap is a fundamental barrier. While quantization and specialised silicon are narrowing the gap, the current state of the art does not yet meet the < 1 nJ/bit target required for mass-market IoT deployment in 6G.

**5. Training instability and hyperparameter sensitivity.** End-to-end autoencoder training is sensitive to learning rate schedules, network depth, batch size, and training SNR distribution. Performance reported in the literature is often achieved under specific hyperparameter configurations that do not generalise across architectures or channel models. The proof-of-concept benchmark in Section III.A.7 illustrates this sensitivity: with a modest training budget, the autoencoder fails to converge for all but the smallest block length.

These limitations do not negate the transformative potential described throughout this survey. They do, however, define the research agenda that must be completed before AI-native PHY can be safely and efficiently deployed in commercial 6G networks.
...
7. **Reproducible Reference Benchmark**: BER comparison of autoencoders vs. conventional codes (Turbo, LDPC, Polar) in the short block-length regime for URLLC (Section III.A.7, Table II), constituting the original experimental contribution of this survey. **Critically, the benchmark demonstrates that under-resourced CPU-only training yields performance inferior to conventional codes across all block lengths tested ($n \in \{7,16,32,64\}$), with gaps of 3.91–>14.67 dB relative to the PPV bound. This result is interpreted as evidence of training resource sensitivity, not as evidence of architectural inferiority, and underscores the need for GPU-scale training and reproducible evaluation in future AI-PHY benchmarking.**

8. **Critical Limitations Analysis**: A structured assessment of known limitations (Section VI, Preamble) identified five key barriers to deployment: (1) poor generalisation to out-of-distribution channels; (2) sample inefficiency relative to classical design; (3) lack of interpretability and formal certification; (4) hardware/energy constraints in IoT devices; and (5) training instability and hyperparameter sensitivity. These limitations are not dismissed but are positioned as the primary research agenda for the community.

### B. Potential Impact on 6G

Native AI at the physical layer represents a paradigm shift with transformative impact:

**Conditional Performance Improvements**: The benchmarks presented in this survey (Sec. III.A.7, Table II) together with results from the literature provide a nuanced picture:
- **Short block-length coding**: Published literature [9], [10] demonstrates that *fully-trained* autoencoders (GPU-scale, millions of epochs) can achieve competitive or superior BER relative to Polar and Turbo codes for $n \leq 32$. Our proof-of-concept benchmark (Table II) demonstrates that, with limited CPU training (~4 s per configuration), the autoencoder requires 11.54 dB vs. 3.89 dB for Polar SC at $n=7$, and fails to converge at $n \geq 16$ — a gap of >7–14 dB from the PPV bound. These results are consistent and reproducible; they confirm that autoencoder-based coding is *training-resource limited*, not *architecturally limited*, but also highlight a critical practical barrier.
- **OFDM channel estimation**: Gains of 2–4 dB in NMSE relative to classical MMSE reported in [29],[31], conditional on channel-matched training.
- **Unrolled WMMSE beamforming**: 10–100× acceleration in convergence with equivalent performance [37].
- **Neural JSCC for images**: 2–3 dB improvement in PSNR relative to JPEG+LDPC at low SNR [48], conditional on matched SNR conditions.
